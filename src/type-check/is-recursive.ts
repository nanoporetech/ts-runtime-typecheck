...
export const isRecordRecursive = memoize(<T> (isType: TypeAssert<T>): TypeAssert<T[]> => {
...
export const isOptRecordRecursive = memoize(<T> (isType: TypeAssert<T>): TypeAssert<Optional<T[]>> => {
  const result = (obj: unknown): obj is T[] => {
